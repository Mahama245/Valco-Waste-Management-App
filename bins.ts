...
// Role hierarchy: only a SUPER_ADMIN may create or promote another account
// to SUPER_ADMIN. ICT_ADMIN can manage every other role but not that one —
// otherwise any ICT_ADMIN could mint themselves unlimited top-level access.
...
async function countActiveSuperAdmins(): Promise<number> {
  const result = await pool.query("SELECT COUNT(*)::int AS n FROM users WHERE role = 'SUPER_ADMIN' AND is_active = true");
...
router.post("/", authenticate, authorize(...ADMIN_ROLES), async (req: AuthedRequest, res) => {
  ...
  const existingUsername = await pool.query("SELECT id, is_active FROM users WHERE username = $1", [cleanUsername]);
  if (existingUsername.rows[0]) {
    const inactiveNote = existingUsername.rows[0].is_active ? "" : " (that account exists but is currently suspended — reactivate it instead of creating a new one, or pick a different username).";
    return res.status(409).json({ error: `The username "${cleanUsername}" is already taken.${inactiveNote}` });
  }
  if (cleanEmail) {
    const existingEmail = await pool.query("SELECT id FROM users WHERE email = $1", [cleanEmail]);
    if (existingEmail.rows[0]) {
      return res.status(409).json({ error: `The email "${cleanEmail}" is already used by another account.` });
    }
  }
  ...
  try {
    const result = await pool.query(
      ...
      [full_name, cleanUsername, cleanEmail, hash, role, department, zone_id]
    );
    await logAudit({
      userId: req.user!.id,
      action: "CREATE",
...
router.patch("/:id/deactivate", authenticate, authorize(...ADMIN_ROLES), async (req: AuthedRequest, res) => {
  ...
  if (target.rows[0].role === "SUPER_ADMIN" && (await countActiveSuperAdmins()) <= 1) {
    return res.status(400).json({ error: "Can't deactivate the last active Super Admin account." });
  }
...
router.patch("/:id", authenticate, authorize(...ADMIN_ROLES), async (req: AuthedRequest, res) => {
  ...
  if (role && !canAssignRole(req.user!.role, role)) {
    return res.status(403).json({ error: "Only a Super Admin can promote an account to Super Admin." });
  }
  if (role && role !== "SUPER_ADMIN" && before.rows[0].role === "SUPER_ADMIN" && (await countActiveSuperAdmins()) <= 1) {
    return res.status(400).json({ error: "Can't change the role of the last active Super Admin account." });
  }
...
router.delete("/:id", authenticate, authorize(...ADMIN_ROLES), async (req: AuthedRequest, res) => {
  ...
  if (!target.rows[0]) return res.status(404).json({ error: "User not found." });
  if (target.rows[0].role === "SUPER_ADMIN" && (await countActiveSuperAdmins()) <= 1) {
    return res.status(400).json({ error: "Can't delete the last active Super Admin account." });
  }
  try {
