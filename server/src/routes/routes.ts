...
router.post("/", authenticate, authorize(...MANAGE_ROLES), async (req: AuthedRequest, res) => {
  ...
  const countRes = await pool.query("SELECT COUNT(*)::int AS n FROM routes");
  const code = `RT-2026-${String(countRes.rows[0].n + 1).padStart(3, "0")}`;
...
router.patch("/:id/start", authenticate, async (req: AuthedRequest, res) => {
...
router.post("/sync", authenticate, async (req: AuthedRequest, res) => {
